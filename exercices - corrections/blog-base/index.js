...
// app.get('/coucou/:name', (req, res, next) => {
...
app.post('/connecter-verif', (req, res, next) => {
    if (!req.body.pseudo || !req.body.mdp) {
        app.locals.message = { class: 'warning', texte: 'Pseudo et/ou le mot de passe non fourni(s).' };
        ...
        MongoClient.connect(urlDB, { useUnifiedTopology: true }, (err, client) => {
            ...
            collection.find({ pseudo: req.body.pseudo, mdp: req.body.mdp }).toArray((err, data) => {
                if (data.length) {
                    req.session.utilisateur.pseudo = data[0].pseudo;
                    req.session.utilisateur.niveau = data[0].niveau;
                    textesPug.utilisateur = req.session.utilisateur;
                    textesPug.message = { class: 'success', texte: 'Vous êtes bien connecté' };
                    textesPug.titre = `Accueil de l'administration`;
                    res.render('admin-accueil',textesPug);
                } else {
                    app.locals.message = { class: 'danger', texte: 'Pseudo et/ou le mot de passe incorrect(s).' };
                    res.redirect('/connecter');
                }
            });
        });
...
app.post('/admin/ajout-utilisateur-verif', (req, res, next) => {
    ...
    if (!req.body.pseudo || !req.body.mdp || [1,5,10].indexOf(niveau) == -1) {
        app.locals.message = { class: 'warning', texte: 'Il manque des informations.' };
        res.redirect('/admin/ajout-utilisateur');
    } else {
        MongoClient.connect(urlDB, { useUnifiedTopology: true }, (err, client) => {
            if (err) return;
            const collection = client.db(nameDb).collection('utilisateurs');
            collection.insertOne({ pseudo: req.body.pseudo, mdp: req.body.mdp, niveau: niveau, prenom: req.body.prenom, nom: req.body.nom, rue: req.body.rue, cp: req.body.cp, ville: req.body.ville, telephone: req.body.telephone }, (err, r) => {
                client.close();
                if (err) {
                    app.locals.message = { class: 'danger', texte: `Le nouvel utilisateur n'a pas pu être enregistré.` };
                    res.redirect('/admin/ajout-utilisateur');
                } else {
                    app.locals.message = { class: 'success', texte: `Le nouvel utilisateur a été enregistré.` };
    ...
    if (!req.body.pseudo || !req.body.mdp || [1,5,10].indexOf(niveau) == -1) {
        ...
        MongoClient.connect(urlDB, { useUnifiedTopology: true }, (err, client) => {
            ...
            collection.find({ pseudo: req.body.pseudo }).toArray((err, data) => {
                if (data.length) {
                    collection.updateOne({pseudo:req.body.pseudo},{ $set: { pseudo: req.body.pseudo, mdp: req.body.mdp, niveau: niveau, prenom: req.body.prenom, nom: req.body.nom, rue: req.body.rue, cp: req.body.cp, ville: req.body.ville, telephone: req.body.telephone } }, (err, r) => {
                        client.close();
                        if (err) {
                            app.locals.message = { class: 'danger', texte: `L'utilisateur ${req.body.pseudo} n'a pas pu être modifié.` };
                            res.redirect('/admin/liste-utilisateur');
                        } else {
                            app.locals.message = { class: 'success', texte: `L'utilisateur ${req.body.pseudo} a pu être modifié.` };
                            res.redirect('/admin/liste-utilisateur');
                        }
                    });
                } else {
                    // Si on ne trouve pas un utilisateur dans la bdd, on redirige vers la liste des utilisateurs avec un message d'avertissement
                    app.locals.message = { class: 'danger', texte: `L'utilisateur à mofidier est inconnu.` };
                    res.redirect('/admin/liste-utilisateur');
                }
            });
        });
        ...
        collection.find({ pseudo: req.params.pseudo }).toArray((err, data) => {
            if (data.length) {
                collection.deleteOne({ pseudo: req.params.pseudo }, (err, r) => {
                    app.locals.message = { class: 'success', texte: `L'utilisateur à bien été supprimé.` };
                    client.close();
                    res.redirect('/admin/liste-utilisateur'); 
                });
            } else {
                client.close();
                app.locals.message = { class: 'danger', texte: `L'utilisateur à supprimer est inconnu.` };
                res.redirect('/admin/liste-utilisateur');
            }
        });
    });
