...
app.get('/mongo', (req, res) => {
  ...
  mongoClient.connect(function(err, client) {
    if (err) {
      ...
      // Un autre exemple, cette fois-ci avec la méthode findOne() et en renvoyant dynamiquement des données dans le navigateur...
...
// Deuxième méthode : on définit une route pour /route-form et on enchaîne avec les différentes méthodes et leur callback respective...
...
// Troisième méthode : on utiliser app.all pour gérer les requêtes HTTP sur la route /all-form peu importe la méthode utilisée... ATTENTION ! Cette méthode gèrera également des requêtes HTTP avec des méthodes autres que GET et POST !
...
app.get('/coucou/:name',
  ...
  (req, res) => {
    ...
    res.send('Route /coucou/:name appelée avec un nom autre que Toto ! Bienvenue sur le site, ' + req.params.name);
  }
);
...
app.use('/pages', (req, res) => {
  ...
  } else if (pageToDisplay === 'newPage.pug') {
    optionsObj = {}
  } else {
    pageToDisplay = 'erreur.pug';
    optionsObj = {};
  }
...
app.use((err, req, res, next) => {
  ...
  // Le code qui suit n'est pas nécessaire, mais sert simplement à montrer que l'on peut faire ce que l'on veut avec l'objet d'erreur créé plus haut dans la route de gestion des routes non prévues par le code...
  if (!err?.redirection) {
...
// de type http.Server...
const server = app.listen(8070, '127.0.0.1', () => {
